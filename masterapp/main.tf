...
data "template_file" "task_def" {
  template = "${file("${path.module}/task_def_wazuh_master.json")}"
  vars {
#  elasticsearch_node  = "${data.terraform_remote_state.loadbalancers.internal_nlb_dns_name??}"
  logstash_node  = "${data.terraform_remote_state.cluster.logstash_internal_elb_dns_name}"
  }
}


module "masterapp" {
  source                    = "../modules/services/masterapp"
...
terraform {
  backend "s3" {
    ...
    key = "dev/elktest/masterapp/terraform.tfstate"
    region = "us-east-1"
  }
}
